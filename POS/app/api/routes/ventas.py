...
async def procesar_venta(
    ...
    try:
        ...
        nueva_venta = Venta(
            ...
            status_pago="pagado"  # Marcar como pagado inmediatamente
        )
...
async def listar_ventas(
    ...
    - ⚡ OPTIMIZADO: Sin eager loading de detalles (no se usan en listado)
    """
    from datetime import datetime
    from sqlalchemy import func
    
    # ⚡ OPTIMIZACIÓN: Query con COUNT subquery para evitar N+1
    statement = (
        select(
            Venta,
        ...
        .where(Venta.tienda_id == current_tienda.id)
        .group_by(Venta.id)
    )
    ...
    # Ordenar y paginar
    statement = statement.order_by(Venta.fecha.desc()).offset(skip).limit(limit)
    
    result = await session.execute(statement)
    rows = result.all()
    
    # Construir respuesta con conteo de items (sin queries adicionales)
    ventas_response = [
        VentaListRead(
            id=venta.id,
            fecha=venta.fecha,
